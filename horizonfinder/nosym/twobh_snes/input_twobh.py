...
def hguess(theta, phi):
    s = 2 # scale 
    R0 = 0.5 * (M1 + M2) # base radius ~ combined mass scale
    eps = 0.2 # small oblateness (helps convergence)
    P2 = 0.5 * (3*np.cos(theta)**2 - 1) # deformation term (Legendre P2)
    r = s * R0 * (1 + eps * P2) # final shape
    return r
